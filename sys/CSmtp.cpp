...
void CSmtp::Send()
{
    ...
    try {
        ...
        for(FileId=0; FileId<Attachments.size(); FileId++) {
            ...
            EncodedFileName += Base64Encode((unsigned char *) FileName.c_str(), FileName.size());
            EncodedFileName += "?=";
            ...
            for(i=0; i<FileSize/54+1; i++) {
                ...
                MsgPart ? strcat(SendBuf,Base64Encode(reinterpret_cast<const unsigned char*>(FileBuf),res).c_str())
                : strcpy(SendBuf,Base64Encode(reinterpret_cast<const unsigned char*>(FileBuf),res).c_str());
                strcat(SendBuf,"\r\n");
                MsgPart += res + 2;
		...
		if(hFile) fclose(hFile); hFile = NULL;
		if(FileBuf) delete[] FileBuf; FileBuf = NULL;
    ...
    try {
        ...
        if(authenticate && IsKeywordSupported(RecvBuf, "AUTH") == true) {
            ...
            if(IsKeywordSupported(RecvBuf, "LOGIN") == true) {
                ...
                // send login:
                std::string encoded_login = Base64Encode(reinterpret_cast<const unsigned char*>(m_sLogin.c_str()),m_sLogin.size());
                ...
                // send password:
                std::string encoded_password = Base64Encode(reinterpret_cast<const unsigned char*>(m_sPassword.c_str()),m_sPassword.size());
                ...
                unsigned char *ustrLogin = (unsigned char*)(SendBuf);
                for(unsigned int i=0; i<length; i++) {
                    if(ustrLogin[i]==94) ustrLogin[i]=0;
                }
                std::string encoded_login = Base64Encode(ustrLogin, length);
                //delete[] ustrLogin;
                ...
                std::string decoded_challenge = Base64Decode((const unsigned char*)encoded_challenge.c_str(),encoded_challenge.length());
                ...
                if(decoded_challenge.empty() || m_sPassword.empty())
                    throw ECSmtp(ECSmtp::BAD_LOGIN_PASSWORD);


                // if ustrPassword is longer than 64 bytes reset it to ustrPassword=MD5(ustrPassword)
                std::string md5pwd = m_sPassword;
                int passwordLength=m_sPassword.size();
                unsigned char tmpchar[MD5_DIGEST_LENGTH];
                if(passwordLength > 64) {
                    MD5((const unsigned char*)m_sPassword.c_str(),passwordLength,tmpchar);
                    md5pwd = (char*)tmpchar;
                    passwordLength = 16;
                }
                ...
                memcpy(ipad, md5pwd.c_str(), passwordLength);
                memcpy(opad, md5pwd.c_str(), passwordLength);
                ...
                //perform inner MD5
                unsigned char ustrResult[MD5_DIGEST_LENGTH];
                MD5_CTX md5pass1;
                MD5_Update(&md5pass1,ipad, 64);
                MD5_Update(&md5pass1,(const unsigned char*)decoded_challenge.c_str(), decoded_challenge.size());
                MD5_Final(ustrResult,&md5pass1);

                //perform outer MD5
                MD5_CTX md5pass2;
                MD5_Update(&md5pass2,opad, 64);
                MD5_Update(&md5pass2,ustrResult, 16);
                MD5_Final(ustrResult,&md5pass1);
                decoded_challenge = (char*)ustrResult;
                ...
                encoded_challenge = Base64Encode(reinterpret_cast<const unsigned char*>(decoded_challenge.c_str()),decoded_challenge.size());
                ...
                if (realm.empty() || m_sLogin.empty() || m_sPassword.empty() || nonce.empty() || uri.empty() || qop.empty()) {
                    throw ECSmtp(ECSmtp::BAD_LOGIN_PASSWORD);
                }

                unsigned char ua1[MD5_DIGEST_LENGTH];//生成的md5值,16字节
                MD5_CTX md5a1a;
                MD5_Init(&md5a1a);
                MD5_Update(&md5a1a,(const unsigned char*)m_sLogin.c_str(), m_sLogin.size());
                MD5_Update(&md5a1a,(unsigned char*)":", 1);
                MD5_Update(&md5a1a,(const unsigned char*)realm.c_str(), realm.size());
                MD5_Update(&md5a1a,(unsigned char*)":", 1);
                MD5_Update(&md5a1a,(const unsigned char*)m_sPassword.c_str(), m_sPassword.size());
                MD5_Final(ua1,&md5a1a);

                unsigned char a1[MD5_DIGEST_LENGTH];//生成的md5值,16字节
                MD5_CTX md5a1b;
                MD5_Init(&md5a1b);
                MD5_Update(&md5a1b,ua1, 16);
                MD5_Update(&md5a1b,(unsigned char*)":", 1);
                MD5_Update(&md5a1b,(const unsigned char*)nonce.c_str(), nonce.size());
                MD5_Update(&md5a1b,(unsigned char*)":", 1);
                MD5_Update(&md5a1b,(unsigned char*)cnonce, strlen(cnonce));
                //authzid could be added here
                MD5_Final(a1,&md5a1a);

                unsigned char a2[MD5_DIGEST_LENGTH];//生成的md5值,16字节
                MD5_CTX md5a2;
                MD5_Init(&md5a2);
                MD5_Update(&md5a2,(unsigned char*) "AUTHENTICATE:", 13);
                MD5_Update(&md5a2,(const unsigned char*)uri.c_str(), uri.size());
                ...
                MD5_CTX md5;
                MD5_Init(&md5);
                MD5_Update(&md5,a1, 32);
                MD5_Update(&md5,(unsigned char*)":", 1);
                MD5_Update(&md5,(const unsigned char*)nonce.c_str(), nonce.size());
                MD5_Update(&md5,(unsigned char*)":", 1);
                MD5_Update(&md5,(unsigned char*)nc, strlen(nc));
                MD5_Update(&md5,(unsigned char*)":", 1);
                MD5_Update(&md5,(unsigned char*)cnonce, strlen(cnonce));
                MD5_Update(&md5,(unsigned char*)":", 1);
                MD5_Update(&md5,(const unsigned char*)qop.c_str(), qop.size());
                MD5_Update(&md5,(unsigned char*)":", 1);
                MD5_Update(&md5,a2, 32);
                static unsigned char decryptstr[MD5_DIGEST_LENGTH];//生成的md5值,16字节
                MD5_Final(decryptstr,&md5);
                decoded_challenge = (char*)decryptstr;
                ...
                strcat(SendBuf, RecvBuf);
                encoded_challenge = Base64Encode((unsigned char*)SendBuf, strlen(SendBuf));
                snprintf(SendBuf, BUFFER_SIZE, "%s\r\n", encoded_challenge.c_str());
                pEntry = FindCommandEntry(command_DIGESTMD5);
                SendData(pEntry);
                ReceiveResponse(pEntry);
