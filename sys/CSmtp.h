/***************************************
* @file     CSmtp.h
* @brief    C++跨平台发送邮件类。出处：http://www.codeproject.com/Articles/98355/SMTP-Client-with-SSL-TLS
* @details  普通的邮件可通过SMTP协议发送。但现在主流邮箱商都要求SSL加密链接，所以SMTP必须使用SSL加密。
            测试Gmail, 126, 163, QQ 邮箱都可行
			注意：依赖于base64编解码，可使用common/charset中的函数
			      依赖于openssl库
* @author   phata, wqvbjhc@gmail.com
* @date     2014-4-2
* @mod      2014-4-2  phata  使用openssl中的md5替换作者自己写的md5.
                             CharToUnsignedChar多余，直接强制转换就行
****************************************/
//////////////////////////////////////////////////////////////////////
...
#define MSG_SIZE_IN_MB 100		// the maximum size of the message with all attachments
#define COUNTER_VALUE	100		// how many times program will try to receive data

const char BOUNDARY_TEXT[] = "__MESSAGE__ID__89yg6f6h6jhc6345";
...
public:
    ...
    CSmtpError GetErrorNum(void) const {
        return ErrorCode;
    }
...
#define test_qqmail_tls

#if defined(test_gmail_tls)//成功
        mail.SetSMTPServer("smtp.gmail.com",587);
        mail.SetSecurityType(USE_TLS);
#elif defined(test_126mail_tls)//成功
        mail.SetSMTPServer("smtp.126.com",25);
        mail.SetSecurityType(USE_TLS);
#elif defined(test_163mail_tls)//成功
        mail.SetSMTPServer("smtp.163.com",465);
        mail.SetSecurityType(USE_SSL);
#elif defined(test_qqmail_tls)//成功.设置了独立密码则必须使用独立密码才行
        ...
        mail.SetLogin("wqvbjhc@gmail.com");
        mail.SetPassword("wqvb30141592653jhc");
        mail.SetSenderMail("wqvbjhc@gmail.com");


        mail.SetLogin("wqvbjhc@126.com");
        mail.SetPassword("wqvb30141592653jhc");
        mail.SetSenderMail("wqvbjhc@126.com");

        mail.SetLogin("wqvbjhc@163.com");
        mail.SetPassword("301415phata");
        mail.SetSenderMail("wqvbjhc@163.com");

        mail.SetLogin("449339052@qq.com");
        mail.SetPassword("30141592653");
        mail.SetSenderMail("449339052@qq.com");

        mail.SetSenderName("wqvbjhcgmail");
        mail.SetReplyTo("wqvbjhc@126.com");
        mail.SetSubject("The message");
        mail.AddRecipient("1930952842@qq.com");
        //mail.AddRecipient("449339052@qq.com");
