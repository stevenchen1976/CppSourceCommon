/***************************************
* @file     packet_async.h
* @brief    TCP 数据包封装.依赖libuv,openssl.功能：接收数据，解析得到一帧后回调用户去处理。异步处理，接收与解析在不同线程
...
* @mod      2014-07-24 phata 添加GetUVError，PacketData两个公共函数. TCP Server Client会调用到
			...
			2014-08-25 phata packet.h更名为packet_async.h
****************************************/
#ifndef PACKET_ASYNC_H
#define PACKET_ASYNC_H
#include <algorithm>
#include "uv.h"
...
{
public:
    PacketAsync(): packet_cb_(NULL), packetcb_userdata_(NULL)
...
protected:
    static void ParseThread(void* arg) {
        PacketAsync* theclass = (PacketAsync*)arg;
        if (!theclass) {
            return;
        }
...
private:// no copy
    PacketAsync(const PacketAsync&);
    PacketAsync& operator = (const PacketAsync&);
};
