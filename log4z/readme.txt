该目录下存放log4z的源代码。
对于日志库的选择，一直在徘徊，以前经过多数对比选择了log4c,但使用过程中还是不满足我的要求。对于7*24的服务端来说，产生的日志是大量，且需要按日期分类。不知是我不会使用，几个log库都找不到有按日期分类的功能。最终找到了一个轻量级的log4z库。

许可协议：https://github.com/zsummer/log4z
MIT open source license,very liberal.

#########################################IDE环境设置########################################################################
codeblocks添加log4c变量=$(CODEBLOCKS)\sdk\log4c
头文件搜索路径$(#log4c)\include
库文件搜索路径$(#log4c)\lib\gcc_lib
在win系统中，运行setx log4c %CD%
vs中项目属性->C++属性中附加$(log4c)\include目录
vs中项目属性->链接器中附加$(log4c)\lib\vc_lib这个目录


####################################################2.5(2014-03-05)##################################################
添加format功能，现在log即支持printf格式，也cout格式。

####################################################2.4(2013-10-17)##################################################
log4z添加日志分块的功能。SetLoggerLimitSize函数。默认100M。
弃用自己的修改，使用源作者的。不然源作者一升级我这里更新麻烦。

####################################################2.3.1(2013-9-17)##################################################
log4z添加了SetLoggerMonthdir功能，日志时间更精确。

####################################################2.3(2013-9-12)##################################################
log4z支持按月创建目录，在配置文件中monthdir。但它的main logid却默认不开启，且没有函数可设置，只能通过配置文件设置。
我仿照它的SetLoggerLevel，SetLoggerDisplay，添加了SetLoggerMonthdir。 这样就可以设置不同logid是否开启每月创建目录的功能。
...
complete priority level control.
完整的日志优先级控制.
multi log output, thread safe.
多日志分流 线程安全.
...
启动log4z日志系统[Start the log4z log system]
ILog4zManager::GetInstance()->Start();
现在开始已经可以在任意线程内很优雅的使用[Now already can be used in arbitrary thread is elegant]
...
结束操作: 不需要做任何销毁操作, 在程序正常退出的情况下(例如 非崩溃 非系统异常断电关机) log4z会安全的自我销毁[The end of operation: don't need to do any destruction in operation, from the normal procedures circumstances (such as non collapse not abnormal power off) log4z safety self destruction.].
