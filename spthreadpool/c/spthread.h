...
extern "C" {
#endif
#define inline __inline//CÖÐÃ»inline
#ifndef WIN32
...
inline int sp_thread_mutex_init( sp_thread_mutex_t * mutex, void * attr )
{
...
inline int sp_thread_mutex_destroy( sp_thread_mutex_t * mutex )
{
...
inline int sp_thread_mutex_lock( sp_thread_mutex_t * mutex )
{
...
inline int sp_thread_mutex_unlock( sp_thread_mutex_t * mutex )
{
...
inline int sp_thread_cond_init( sp_thread_cond_t * cond, void * attr )
{
    *cond = CreateEvent( NULL, FALSE, FALSE, NULL );
    return NULL == *cond ? GetLastError() : 0;
}

inline int sp_thread_cond_destroy( sp_thread_cond_t * cond )
{
...
*/
inline int sp_thread_cond_wait( sp_thread_cond_t * cond, sp_thread_mutex_t * mutex )
{
    int ret = 0;
...
inline int sp_thread_cond_signal( sp_thread_cond_t * cond )
{
    int ret = SetEvent( *cond );
    return 0 == ret ? GetLastError() : 0;
}

inline sp_thread_t sp_thread_self()
{
    return GetCurrentThreadId();
}

inline int sp_thread_attr_init( sp_thread_attr_t * attr )
{
    *attr = 0;
    return 0;
}

inline int sp_thread_attr_setdetachstate( sp_thread_attr_t * attr, int detachstate )
{
    *attr |= detachstate;
    return 0;
}

inline int sp_thread_create( sp_thread_t * thread, sp_thread_attr_t * attr,
